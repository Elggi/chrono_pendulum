...
class CalibrationNode(Node):
    ...
    def run_protocol(self, csv_path):
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            ...
            def two_turn_segment(direction: float, seg_name: str, turns: float = 2.0):
                ...
                target_delta = turns * 2.0 * math.pi

                while time.time() < t_timeout:
                    signed_progress = (yaw_unwrapped - yaw_start) * direction
                    if signed_progress >= target_delta:
                        break
            ...
            two_turn_segment(1.0, "forward_2turn", turns=2.0)

            t_pause = time.time() + self.args.dir_pause_sec
            while time.time() < t_pause:
                step_io("dir_pause", 0.0)

            two_turn_segment(-1.0, "reverse_2turn", turns=2.0)
