...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage1 PEM standalone entrypoint")
    ...
    (args.outdir / "config_used.json").write_text(
        json.dumps({"stage": 1, "csv": str(args.csv), "meta": str(args.meta), "weights": weights}, indent=2),
        encoding="utf-8",
    )
    print(f"[DONE] Stage1 PEM artifacts: {args.outdir}")
