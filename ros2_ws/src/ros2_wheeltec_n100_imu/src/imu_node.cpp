...
// modified for safer serial parsing / covariance init
// ---------------------------------------------------
...
public:
  ...
  : Node("imu_node")
  {
    ...
    if (imu_mag_cov.size() < 3) {
      imu_mag_cov = {0.0, 0.0, 0.0};
    }
    if (imu_gyro_cov.size() < 3) {
      imu_gyro_cov = {0.0, 0.0, 0.0};
    }
    if (imu_accel_cov.size() < 3) {
      imu_accel_cov = {0.0, 0.0, 0.0};
    }
    mag_covariance_ = 0.0;
...
private:
  ...
  void read_imu()
  {
    ...
    while (serial_.available() > 0)
    {
      ...
      {
        RCLCPP_WARN_STREAM_SKIPFIRST(this->get_logger(), "head_type error: " << (int)head_type[0]);
        continue;
      }

      uint8_t check_len[1] = {0xff};
      if (!read_exact(check_len, 1)) {
        return;
      }
      ...
      if (head_type[0] == TYPE_IMU && check_len[0] != IMU_LEN) {
        ...
        size_t ignore_n = static_cast<size_t>(check_len[0]) + 4;
        std::vector<uint8_t> ground_ignore(ignore_n, 0);
        if (ignore_n > 0) {
          size_t got = serial_.read(ground_ignore.data(), ignore_n);
          (void)got;
        }
        continue;
      }
      ...
      if (!read_exact(check_sn, 1)) return;
      if (!read_exact(head_crc8, 1)) return;
      if (!read_exact(head_crc16_H, 1)) return;
      if (!read_exact(head_crc16_L, 1)) return;
      ...
      if (head_type[0] == TYPE_IMU)
      {
        ...
        if (data_s != (IMU_LEN + 1)) {
          RCLCPP_WARN_STREAM_SKIPFIRST(this->get_logger(), "imu payload short read: " << data_s);
          continue;
        }
      ...
      else if (head_type[0] == TYPE_AHRS)
      {
        ...
        if (data_s != (AHRS_LEN + 1)) {
          RCLCPP_WARN_STREAM_SKIPFIRST(this->get_logger(), "ahrs payload short read: " << data_s);
          continue;
        }
      ...
      else if (head_type[0] == TYPE_INSGPS)
      {
        ...
        if (data_s != (INSGPS_LEN + 1)) {
          RCLCPP_WARN_STREAM_SKIPFIRST(this->get_logger(), "insgps payload short read: " << data_s);
          continue;
        }
